...
def _street_start_index(state: NLHEState) -> int:
...
def _street_actors(state: NLHEState) -> List[int]:
    """
...
def _is_round_complete(state: NLHEState) -> bool:
    ...
    if state.last_raiser >= 0 and state.last_raiser in can_act:
        # Find the last position where last_raiser made a raise (action not in {0,1})
        # street_acts[i] is the actor; the corresponding action is the i-th non-DEAL
        # action in this street's action_history slice.
        street_start = _street_start_index(state)
